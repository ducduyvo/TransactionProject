...
void deleteTransaction(char *fileName1)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (size > 0)
            {
                ...
                while (!feof(file1))
                {
                    if (fread(read, sizeof(transaction), 1, file1) == 1 && read->m_amount > 0)
                    {
                        if (count != index)
                        {
    ...
    else
    {
        ...
        else
        {
            ...
            while (!feof(file2))
            {
                if (fread(read, sizeof(transaction), 1, file2) == 1 && read->m_amount > 0)
                {
...
void addToFile(char *fileName, transaction *arr, int *count)
{
    ...
    else
    {
        ...
        for (int i = 0; i < n; i++)
        {
            if (arr[i].m_amount > 0 && (arr[i].m_type == 1 || arr[i].m_type == -1))
            {
...
transaction *addMoney(transaction *arr, char *description, float amount, char *date, int *count)
{
    ...
    (*count)++;
    if ((arr = realloc(arr, sizeof(transaction) * (*count))) == NULL)
    {
    ...
    while (1)
    {
        ...
        case 6:
            if (arr == NULL || count == 0)
                ...
                arr = calloc(1, sizeof(transaction));
            count = 0;
            break;
        case 7:
...
void printRecord(char *fileName)
{
    ...
    else
    {
        ...
        while (!feof(file))
        {
            if (fread(read, sizeof(transaction), 1, file) == 1 && read->m_amount > 0)
            {
                count++;
